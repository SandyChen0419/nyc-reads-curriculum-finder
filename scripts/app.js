...
(function initApp() {
  ...
  function bindEvents() {
    const runSearch = () => {
      ...
      // Validation: District is OPTIONAL; require school + grade + date
      if (!schoolVal) {
    ...
    // Auto-fill district when school is selected (district optional UX)
    els.schoolInput.addEventListener('change', () => {
      const schoolVal = String(els.schoolInput.value || '').trim();
      if (!schoolVal || !state.meta || !state.meta.districtBySchool) return;
      const d = state.meta.districtBySchool[schoolVal];
      if (d) {
        els.district.value = String(d);
        // Do NOT call onDistrictChange() here to avoid clearing the school input.
      }
    });
